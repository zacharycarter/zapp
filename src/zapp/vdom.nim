when defined emscripten:
  ...
  proc vthunk*(name: string; args: varargs[VNode, vn]): VNode =
    VNode(kind: VNodeKind.vthunk, text: name, index: -1, kids: @args)
  
  proc dthunk*(name: string; args: varargs[VNode, vn]): VNode =
  ...
  #     text: string(astToStr(t)), renderImpl: render,
  ...
  proc takeOverFields*(newNode, oldNode: VNode) =
    ...
    # shallowCopy oldNode.style newNode.style
  ...
  proc text*(s: string): VNode = VNode(kind: VNodeKind.text, text: s, index: -1)
  ...
  proc getVNodeById*(n: VNode; id: string): VNode =
